...
-- Initial schema for the Turkey Mining & Quarrying Accidents Database.
--
-- Evidence flow: source document -> claim -> decision -> canonical value -> export.
-- Design notes:
--   * SQLite now, forward-compatible with PostgreSQL/PostGIS. Coordinates are
--     REAL columns (WGS84), documented as future geometry(Point, 4326).
--   * Stable workflow enums are enforced with CHECK constraints here;
--     vocabulary-driven codes (hazards, admin areas, ...) are validated in the
--     Python layer against data/vocabularies/*.csv (see docs/data_dictionary.md).
--   * Append-only tables get triggers rejecting UPDATE/DELETE. `claims` allows
--     updates ONLY to incident_id (linking) and review_status (workflow);
--     `casualty_observations` only to is_current_canonical and review_status.
--   * Timestamps are ISO 8601 text; audit timestamps UTC, incident times local
--     with explicit offset plus date_precision.
...
-- ---------------------------------------------------------------------------
-- Classification / role / recommendation layer
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Audit / governance layer (append-only)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Aggregate context tables (schema now, data is a future phase).
-- Binding doc rules: raw counts never labeled "risk"; rates only with a
-- documented exposure denominator; coverage analysis is future work.
-- ---------------------------------------------------------------------------
...
-- Mapping between classification systems (deviation from the uniform aggregate
-- template, logged in docs/open_questions.md implementation note B).
...
-- Claim content is immutable evidence. Only incident_id (linking) and
-- review_status (workflow) may change. See docs/open_questions.md note A.
...
-- AI/OCR-assisted claims must enter the review queue (hard constraint 5).
...
-- Decisions are never edited: supersession creates a new row referencing the old.
...
-- Casualty figures are immutable observations; only canonical flag and review
-- status change, and only through review.py acting on a recorded decision.
